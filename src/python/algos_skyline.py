...
def median_absolute_deviation(timeseries):
	...
	# 8 times bigger than the median
	if test_statistic > 8:
			return True
	else:
			return False
...
def stddev_from_moving_average(timeseries):
	...
	return abs(series.iget(-1) - expAverage.iget(-1)) > 3.5 * stdDev.iget(-1)
...
def mean_subtraction_cumulation(timeseries):
	...
	return abs(series.iget(-1)) > 3.5 * stdDev
...
def least_squares(timeseries):
	...
	return abs(t) > std_dev * 4.5 and round(std_dev) != 0 and round(t) != 0
