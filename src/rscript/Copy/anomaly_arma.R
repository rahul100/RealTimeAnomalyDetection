...
while(1){
	...
	cond <- simpleError("error")
        fit =tryCatch( arima(y, c(p, d, q),seasonal = list(order = c(p, d, q), period = 48), method="ML"),error=function(cond) NULL)
        if(is.null(fit)) next
...
#        anomalies_1
        ...
        apply(anomalies_1, 1, writedb ,"arma")
        }
        
        #-------------------------------------------------
        
        time_elapsed<-proc.time() - ptm
        if(time_elapsed>0 && time_elapsed<1) { Sys.sleep(1 - time_elapsed[3]) }  #  sleep for remaining time (algorithm needs to run exactly after 1sec)
